  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 flex flex-col justify-center px-8 max-w-6xl w-full gap-2">
        ...
        <p className="text-left text-white/60 text-base max-w-lg mt-2 leading-relaxed font-sans">
          BS CS Summa Cum Laude · Hackathon Champion · IEEE-published Researcher
          · DOST-SEI Scholar
        </p>

        <div className="w-16 h-px bg-indigo-500 mt-4 mb-5" />
        ...
        <div className="flex flex-row items-center gap-4 flex-wrap">
          <a
            ...
            className="flex items-center gap-2 rounded-full text-indigo-400 border-2 border-indigo-500 w-fit text-sm font-mono py-2 px-5 hover:bg-indigo-500 hover:text-white transition-all duration-200"
          >
            <FileText size={16} />
            Resume
          </a>
          ...
          <div className="flex items-center gap-3">
            {socials.map(({ label, href, icon: Icon }) => (
              <a
                ...
                rel="noopener noreferrer"
                aria-label={label}
                className="flex items-center justify-center w-9 h-9 rounded-full border border-white/20 text-white/60 hover:text-white hover:border-indigo-400 hover:bg-indigo-500/50 transition-all duration-200"
              >
                <Icon size={18} />
              </a>
            ))}
        ...
        <div className="mt-12 flex items-center gap-2 text-white/30 text-xs font-mono animate-bounce">
          <ArrowDown size={14} />
          scroll to explore
        </div>
      </div>

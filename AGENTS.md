...
- **Dual-Font System (Strict Rule):**
  - **Monospace (`JetBrains Mono` / `Fira Code`):** Use ONLY for section headers, terminal command prompts (`> sys.whoami()`), code snippets, badges, and retro pills.
  - **Sans-Serif (`Inter` / `Space Grotesk`):** Use for all long body text, project descriptions, and bio narratives. Do NOT use pixel or monospace fonts for long body text.
- **Toggleable CRT / Light Mode Theme:**
